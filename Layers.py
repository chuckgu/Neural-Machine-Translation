...
class BiDirectionLSTM(object):
    ...
    def _fstep(self, x_t, h_tm1, c_tm1): 
        i_t = T.nnet.sigmoid(T.dot(x_t, self.W_i) + self.b_i + T.dot(h_tm1, self.U_i))
        f_t = T.nnet.sigmoid(T.dot(x_t, self.W_f) + self.b_f + T.dot(h_tm1, self.U_f))
        c_t = f_t * c_tm1 + i_t * T.tanh(T.dot(x_t, self.W_c) + self.b_c + T.dot(h_tm1, self.U_c))
        o_t = T.nnet.sigmoid( T.dot(x_t, self.W_o) + self.b_o + T.dot(h_tm1, self.U_o))
        h_t = o_t * T.tanh(c_t)
    ...
    def _bstep(self, x_t, h_tm1, c_tm1): 
        i_t = T.nnet.sigmoid(T.dot(x_t, self.Wb_i) + self.bb_i + T.dot(h_tm1, self.Ub_i))
        f_t = T.nnet.sigmoid(T.dot(x_t, self.Wb_f) + self.bb_f + T.dot(h_tm1, self.Ub_f))
        c_t = f_t * c_tm1 + i_t * T.tanh(T.dot(x_t, self.Wb_c) + self.bb_c + T.dot(h_tm1, self.Ub_c))
        o_t = T.nnet.sigmoid( T.dot(x_t, self.Wb_o) + self.bb_o + T.dot(h_tm1, self.Ub_o))
        h_t = o_t * T.tanh(c_t)
...
class BiDirectionGRU(object):
    ...
    def _fstep(self,x_t,x_m, h_tm1):
        ...
        z = T.nnet.sigmoid(T.dot(x_t, self.W_z) + self.b_z + T.dot(h_tm1, self.U_z))
        r = T.nnet.sigmoid(T.dot(x_t, self.W_r) + self.b_r + T.dot(h_tm1, self.U_r))
        hh_t = T.tanh(T.dot(x_t, self.W_h) + self.b_h + T.dot(r * h_tm1, self.U_h))
        h_t = z * h_tm1 + (1 - z) * hh_t
    ...
    def _bstep(self,x_t,x_m, h_tm1):
        ...
        z = T.nnet.sigmoid(T.dot(x_t, self.Wb_z) + self.bb_z + T.dot(h_tm1, self.Ub_z))
        r = T.nnet.sigmoid(T.dot(x_t, self.Wb_r) + self.bb_r + T.dot(h_tm1, self.Ub_r))
...
=======
    ...
    def _step(self,y_tm1,y_m,s_tm1,h,x_m):
        ...
        return T.cast(s_t,dtype =theano.config.floatX),T.cast(logit,dtype =theano.config.floatX),c  
    ...
    def get_sample(self,y,s_tm1):
        ...
        h,logit,ctx=self._step(y,y_mask,s_tm1,c,X_mask) 
    ...
    def get_output(self,y,y_mask,init_state):
        ...
        [h,logit,ctx], _ = theano.scan(self._step, 
                                     sequences = [y,y_mask],
